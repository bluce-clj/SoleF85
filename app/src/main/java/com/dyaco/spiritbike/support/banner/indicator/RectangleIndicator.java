...
public class RectangleIndicator extends BaseIndicator {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        setMeasuredDimension(space + normal + config.getSelectedWidth(), (int) BannerUtils.dp2px(16));
    }
...
//            int indicatorHeight = config.getCurrentPosition() == i ? 48 : config.getHeight();
//            // rectF.set(left, 0, left + indicatorWidth, config.getHeight());
//            rectF.set(left, 20, left + indicatorWidth, indicatorHeight);
//
            int indicatorWidth;
            int indicatorHeight;
            if (config.getCurrentPosition() == i) {
                ...
                indicatorHeight = (int)BannerUtils.dp2px(16);
                rectF.set(left, 0, left + indicatorWidth, indicatorHeight);
            } else {

                int height = getHeight() / 2;

                indicatorWidth = config.getNormalWidth();
                indicatorHeight = config.getHeight();

                int t = ((int)BannerUtils.dp2px(16) / 2) - (indicatorHeight / 2);
                int b = ((int)BannerUtils.dp2px(16) / 2) + (indicatorHeight / 2) ;

              //  rectF.set(left, (int)BannerUtils.dp2px(8), left + indicatorWidth, indicatorHeight);
                rectF.set(left, t, left + indicatorWidth, b);
              //  rectF.set(left, height - 100, left + indicatorWidth, indicatorHeight);
            }
